...
export const createProduct = async (req, res) => {
  ...
  if (errors.length) {
    return res
      .status(400)
      .json({ success: false, message: "Payload inválido.", errors });
  }

  const products = await dataService.readAll();
  // percorrer todos os produtos para achar o maior id e somar 1, antes de criar o novo produto
  ...
  if (products.some((p) => normalizeSku(p.sku) === skuNorm)) {
    return res
      .status(409)
      .json({ success: false, message: "Já existe produto com este SKU." });
  }
...
export const updateProduct = async (req, res) => {
  ...
  if (req.body.price !== undefined)
    products[idx].price = Number(req.body.price);
  if (req.body.description !== undefined)
    products[idx].description = String(req.body.description);
