...
export const validateNewProductPayload = (body) => {
  const errors = [];
  // removi a validação do id, pois ele será gerado automaticamente no controller
  if (!body.name || String(body.name).trim().length === 0)
    errors.push("name é obrigatório.");
  if (body.price === undefined || isNaN(Number(body.price)))
    errors.push("price é obrigatório e deve ser numérico.");
...
export const validateUpdateProductPayload = (body) => {
  ...
  if (body.price !== undefined && isNaN(Number(body.price)))
    errors.push("price deve ser numérico.");
  return errors;
};
